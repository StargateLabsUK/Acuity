...
import { ChevronLeft, ChevronRight, ChevronDown, ArrowLeft, ArrowRightLeft } from 'lucide-react';
...
function buildSharedOutcomeLines(cas: CasualtyData, disposition: DispositionType, fields: DispositionFields): string[] {
  const lines: string[] = [`Disposition: ${DISPOSITION_LABELS[disposition]}`];
  if (disposition === 'conveyed') {
    lines.push(`Receiving hospital: ${fields.receiving_hospital || cas.receivingHospital || 'Not confirmed'}`);
    lines.push(`Time of handover: ${fields.time_of_handover || '—'}`);
    lines.push(`Handover given to: ${fields.handover_given_to || '—'}`);
    lines.push(`ePRF handed over: ${fields.eprf_handed_over ? 'Yes' : 'No'}`);
  } else if (disposition === 'see_and_treat') {
    lines.push(`Clinical justification: ${fields.clinical_justification || '—'}`);
    lines.push(`Observations at discharge: ${fields.discharge_observations || '—'}`);
    lines.push(`Advice given: ${fields.advice_given || '—'}`);
    lines.push(`Safety net given: ${fields.safety_net_given ? 'Yes' : 'No'}`);
    lines.push(`Patient understands: ${fields.patient_understands ? 'Confirmed' : 'Not confirmed'}`);
    lines.push(`Time of discharge: ${fields.time_of_discharge || '—'}`);
  } else if (disposition === 'see_and_refer') {
    lines.push(`Clinical justification: ${fields.clinical_justification || '—'}`);
    lines.push(`Observations at discharge: ${fields.discharge_observations || '—'}`);
    lines.push(`Advice given: ${fields.advice_given || '—'}`);
    ...
    lines.push(`Patient understands: ${fields.patient_understands ? 'Confirmed' : 'Not confirmed'}`);
    lines.push(`Time of discharge: ${fields.time_of_discharge || '—'}`);
    lines.push(`Referral destination: ${fields.referral_destination || '—'}`);
    lines.push(`Referral pathway: ${fields.referral_pathway || '—'}`);
    lines.push(`Referral accepted: ${fields.referral_accepted ? 'Yes' : 'No'}`);
    lines.push(`Reference number: ${fields.reference_number || '—'}`);
  } else if (disposition === 'refused_transport') {
    lines.push(`Capacity assessed: ${fields.capacity_assessed ? 'Yes' : 'No'}`);
    lines.push(`Patient has capacity: ${fields.patient_has_capacity ? 'Yes' : 'No'}`);
    lines.push(`Risks explained: ${fields.risks_explained ? 'Yes' : 'No'}`);
    lines.push(`Patient understanding confirmed: ${fields.patient_understanding_confirmed ? 'Yes' : 'No'}`);
    lines.push(`Refusal witnessed by: ${fields.refusal_witnessed_by || '—'}`);
    lines.push(`Time of refusal: ${fields.time_of_refusal || '—'}`);
    lines.push(`Safeguarding concern: ${fields.safeguarding_concern ? 'Yes' : 'No'}`);
    if (!fields.patient_has_capacity) {
      lines.push(`Best interests decision: ${fields.best_interests_decision || '—'}`);
    }
    lines.push(`Signed refusal form: ${fields.signed_refusal_form ? 'Yes' : 'No'}`);
  } else if (disposition === 'role') {
    lines.push(`Time of recognition: ${fields.time_of_recognition || '—'}`);
    lines.push(`Criteria used: ${fields.role_criteria || '—'}`);
    lines.push(`Resuscitation attempted: ${fields.resuscitation_attempted ? 'Yes' : 'No'}`);
    if (fields.resuscitation_attempted) {
      lines.push(`Resuscitation details: ${fields.resuscitation_details || '—'}`);
    }
    lines.push(`GP notified: ${fields.gp_notified ? 'Yes' : 'No'}`);
    lines.push(`Police notified: ${fields.police_notified ? 'Yes' : 'No'}`);
    lines.push(`Coroner referral required: ${fields.coroner_referral ? 'Yes' : 'No'}`);
    lines.push(`Next of kin notified: ${fields.nok_notified ? 'Yes' : 'No'}`);
  }
...
function renderSharedCasualtyOutcome(
  cas: CasualtyData,
  ...
  if (transferInfo) {
    lines.push(`Transferred from: ${transferInfo.from_callsign}`);
    if (transferInfo.accepted_at) {
      lines.push(`Transfer accepted: ${getDateTime(transferInfo.accepted_at)}`);
    }
  }

  if (disposition === 'conveyed') {
    lines.push(`Receiving hospital: ${fields.receiving_hospital || cas.receivingHospital || 'Not confirmed'}`);
    if (fields.time_of_handover) lines.push(`Time of handover: ${fields.time_of_handover}`);
    if (fields.handover_given_to) lines.push(`Handover given to: ${fields.handover_given_to}`);
    return lines;
  }

  if (disposition === 'see_and_treat' || disposition === 'see_and_refer') {
    if (fields.time_of_discharge) lines.push(`Time of discharge: ${fields.time_of_discharge}`);
    if (fields.advice_given) lines.push(`Advice given: ${fields.advice_given}`);
    if (disposition === 'see_and_refer' && fields.referral_destination) {
      lines.push(`Referral destination: ${fields.referral_destination}`);
    }
    return lines;
  }

  if (disposition === 'refused_transport') {
    if (fields.time_of_refusal) lines.push(`Time of refusal: ${fields.time_of_refusal}`);
    lines.push(`Capacity assessed: ${fields.capacity_assessed ? 'Yes' : 'No'}`);
    return lines;
  }

  if (disposition === 'role') {
    if (fields.time_of_recognition) lines.push(`Time of recognition: ${fields.time_of_recognition}`);
    if (fields.role_criteria) lines.push(`Criteria: ${fields.role_criteria}`);
    return lines;
  }
...
function CasualtyReportView({ cas, inc, onBack, onHandover, onTransfer, transferInfo }: {
  ...
  const eprfText = buildCasualtyEprf(cas, inc, disposition, fields);
  const outcomeLines = renderSharedCasualtyOutcome(cas, disposition, fields, transferInfo);
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto px-3 md:px-6 py-3 md:py-5 pb-32">
        <div className="max-w-5xl mx-auto">
          ...
          <PatientReportView
            incidentTitle={formatIncidentTitle(inc.incident_number)}
            incidentDateTime={getDateTime(inc.latest_transmission_at ?? inc.created_at ?? inc.timestamp)}
            casualtyLabel={cas.label}
            casualtyPriority={cas.priority}
            atmist={cas.atmist as Record<string, unknown>}
            latestAbcde={latestAbcde}
            timeline={timelineEntries}
            outcomeLines={outcomeLines}
            eprfText={eprfText}
            atmistFieldRenderer={(field, value) => (
              <EditableField2 value={value} onSave={(nextValue) => saveAtmistField(field, nextValue)} />
            )}
          />
