...
function IncidentCard({ inc, onSelectCasualty, closedKeys }: { inc: Incident; onSelectCasualty: (cas: CasualtyData) => void; closedKeys: Set<string> }) {
  const [expanded, setExpanded] = useState(false);
  const p = inc.assessment?.priority ?? inc.priority ?? 'P3';
  const col = PRIORITY_COLORS[p] ?? '#34C759';
  const casualties = extractCasualties(inc).filter(c => !closedKeys.has(`${inc.id}:${c.key}`));
...
function IncidentDetailView({ inc, onBack, onSelectCasualty, closedKeys }: {
  inc: Incident;
  onBack: () => void;
  ...
  const p = a?.priority ?? inc.priority ?? 'P3';
  const col = PRIORITY_COLORS[p] ?? '#34C759';
  const casualties = extractCasualties(inc).filter(cas => !closedKeys.has(`${inc.id}:${cas.key}`));
  ...
  if (nav.view === 'casualty') {
    return (
      <CasualtyReportView
        ...
        inc={nav.incident}
        onBack={() => {
          const casualties = extractCasualties(nav.incident).filter(c => !closedKeys.has(`${nav.incident.id}:${c.key}`));
