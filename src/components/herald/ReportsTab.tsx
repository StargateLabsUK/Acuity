...
/** Fallback ePRF when full report hasn't synced yet */
function buildFallbackEprf(d: CasualtyDisposition): string {
  const dispLabel = DISPOSITION_LABELS[d.disposition] ?? d.disposition;
  const fields = d.fields ?? {};
  const hospital = (fields as any).receiving_hospital ?? '';
  const notes = (fields as any).notes ?? '';
  let text = `ePRF — PATIENT HANDOVER
═══════════════════════════
PATIENT: ${d.casualty_label}
PRIORITY: ${d.priority}
INCIDENT: ${d.incident_number ?? '—'}
CALLSIGN: ${d.session_callsign ?? '—'}
OUTCOME: ${dispLabel}`;
  if (hospital) text += `\nHOSPITAL: ${hospital}`;
  if (notes) text += `\nNOTES: ${notes}`;
  text += `\n\nHANDED OVER: ${new Date(d.closed_at).toISOString().slice(0, 16).replace('T', ' ')}Z
═══════════════════════════
Full clinical details will appear when the incident report syncs.
  ...
  if (closedCasualties.length === 0 && session) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4">
        <span className="text-lg uppercase font-bold mb-4" style={{ color: '#666666' }}>
                  ...
                  {showEprf && (
                    <div className="mt-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {report && <CopyBtn text={buildClosedEprf(d, report)} label="COPY ePRF" />}
                      </div>
                      <div className="border border-border rounded-lg bg-card p-3">
                        <div className="text-lg text-foreground leading-7 whitespace-pre-wrap break-words">
                          {report ? buildClosedEprf(d, report) : buildFallbackEprf(d)}
                        </div>
                      </div>
                    </div>
                  )}
