...
export function TrainingTab({ reports }: Props) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto px-3 md:px-4 py-3" style={{ scrollbarWidth: 'thin' }}>
      ...
      <p className="text-lg font-bold tracking-[0.25em] mb-3" style={{ color: '#FF9500' }}>
        CORRECTIONS LOG
      </p>

      {editedReports.length === 0 ? (
        <p className="text-lg text-foreground opacity-50 mb-6">No corrected reports yet</p>
      ) : (
        <div className="mb-6">
          {editedReports.map((r) => {
            const diff = (r as any).diff as ReportDiff | null;
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1 flex-wrap mb-2">
                  <span className="text-lg tracking-[0.15em] font-bold" style={{ color: '#FF9500' }}>FIELDS CORRECTED</span>
                ...
                {diff.priority_changed && (
                  <div className="flex items-center gap-2 mb-2 px-2 py-1 rounded" style={{ background: 'rgba(255,149,0,0.08)' }}>
                    <span className="text-lg tracking-[0.15em] font-bold" style={{ color: '#FF9500' }}>PRIORITY CHANGED:</span>
                    <span className="text-lg font-bold" style={{ color: PRIORITY_COLORS[diff.original_priority ?? ''] ?? '#fff' }}>
                      {diff.original_priority}
                    </span>
                    <span className="text-lg text-foreground opacity-50">→</span>
                    <span className="text-lg font-bold" style={{ color: PRIORITY_COLORS[diff.corrected_priority ?? ''] ?? '#fff' }}>
      ...
      {analysing && (
        <div className="flex items-center gap-3 mb-4 px-4 py-3">
          ...
          />
          <span className="text-lg tracking-[0.2em]" style={{ color: '#1E3028' }}>
      ...
      {analysis && (
        ...
        >
          ...
          <p className="text-lg mt-4 opacity-70" style={{ color: '#8A9890' }}>
