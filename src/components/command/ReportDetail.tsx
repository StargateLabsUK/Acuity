...
function CopyBtn({ text, label }: { text: string; label: string }) {
  ...
  return (
    <button
      onClick={copy}
      className="text-sm md:text-lg text-foreground border border-border px-3 py-1.5 rounded-sm bg-transparent cursor-pointer tracking-wide hover:border-primary transition-colors"
    >
      {copied ? 'COPIED' : label}
    </button>
  );
...
CREW NOTES: Generated by Herald Radio Intelligence`;
  ...
  return (
    <div className="h-full overflow-y-auto p-3 md:p-4 flex flex-col gap-4 md:gap-6 min-w-0" style={{ scrollbarWidth: 'thin' }}>
      {/* Priority Banner */}
      <div
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 md:gap-4 min-w-0">
            ...
            <div className="flex items-baseline gap-2 md:gap-3">
              <span className="font-heading text-3xl md:text-5xl leading-none" style={{ color: col }}>
                {priority}
              </span>
          ...
          <div className="text-right">
            <div className="text-sm md:text-xl text-foreground uppercase font-bold">{service}</div>
            <div className="text-xs md:text-lg text-foreground mt-1">{dateStr} {timeStr}</div>
          </div>
        </div>
      </div>
      ...
      {/* Headline */}
      <DetailCard>
        <p className="text-base md:text-xl text-foreground leading-relaxed font-medium break-words">{headline}</p>
      </DetailCard>
      ...
      <div>
        ...
        <DetailCard>
          <p className="text-sm md:text-lg text-foreground leading-7 italic break-words">
            &ldquo;{report.transcript ?? 'N/A'}&rdquo;
          </p>
          <div className="text-xs md:text-lg text-foreground mt-2 md:mt-3 opacity-70">
          ...
          <DetailCard>
            <div className="flex flex-col gap-4">
              {Object.entries(structured).map(([k, v]) => (
                <div key={k}>
                  <div className="text-sm md:text-lg font-bold tracking-wide mb-0.5" style={{ color: col }}>{k}</div>
                  <div className="text-sm md:text-lg text-foreground leading-relaxed break-words">{v ?? '—'}</div>
                </div>
              ))}
              {Object.keys(structured).length === 0 && (
                <span className="text-sm md:text-lg text-foreground opacity-50">No structured fields</span>
              )}
            </div>
          </DetailCard>
        </div>
            ...
            <div className="flex flex-col gap-2">
              {actions.map((act, i) => (
                <div key={i} className="flex gap-3">
                  <span className="text-sm md:text-lg font-bold min-w-[20px]" style={{ color: col }}>{i + 1}.</span>
                  <span className="text-sm md:text-lg text-foreground leading-relaxed break-words">{act}</span>
                </div>
              ))}
            </div>
            {transmitTo && (
              <>
                ...
                <div className="text-sm md:text-lg text-foreground break-words">
      ...
      <div>
        ...
        <DetailCard>
          <div className="text-sm md:text-lg text-foreground leading-7 md:leading-8 whitespace-pre-wrap break-words">
      ...
      <div>
        ...
        <DetailCard>
          <div className="text-sm md:text-lg text-foreground leading-6 md:leading-7 whitespace-pre-wrap break-words">
            {eprfText}
          </div>
        </DetailCard>
      </div>
